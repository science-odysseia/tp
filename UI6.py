...
IMG_PATH = r"./img_files/main_img.png" # assets_dir="img_files" 설정 기준
...
def main(page: ft.Page):
    ...
    def make_bar_chart(series, x_name, y_name, bar_color=ft.Colors.BLUE_400):
        ...
        return fch.BarChart(
            ...
            groups=[fch.BarChartGroup(x=i, rods=[fch.BarChartRod(from_y=0, to_y=float(v), width=25, color=bar_color, border_radius=ft.BorderRadius.only(top_left=6, top_right=6), tooltip=f"{v*100:.1f}%")]) for i, v in enumerate(series.values)],
        )
    ...
    def show_main_screen():
        ...
        main_stack = ft.Stack(
            [
                ft.Image(src=IMG_PATH, fit="cover", width=float("inf"), height=float("inf")),
                ft.Container(
                    content=ft.Column(
                        [
                    ...
                    expand=True,
                    padding=ft.padding.only(left=page.width * 0.1, bottom=page.height * 0.15),
                )
            ],
            expand=True,
        )
    ...
    def show_summary_charts(e):
        ...
        chart1 = fch.PieChart(
            sections=[fch.PieChartSection(value=float(v), title=f"{k}\n{(v/total_dropout*100):.1f}%", color=ft.Colors.BLUE_400 if k == "Male" else ft.Colors.RED_400, radius=50, title_style=ft.TextStyle(size=11, weight="bold", color=ft.Colors.WHITE)) for k, v in dropout_gender.items()],
    ...
    def start_app(e):
        ...
        def update_chart(e=None):
            ...
            elif xk in {"numeric", "binary"} and yk == "categorical": # [이 부분 복구]
...
ft.run(main, assets_dir="img_files")